    ...
    user_prompt = json.dumps(
        {
            ...
            "success_metric_rule": "Every success metric must be phrased as an observable metric using rate, count, number, score, rating, survey, user-reported signal, time, percentage, retention, conversion, decrease, increase, reduction, complaints, reviews, or a numeric unit. A score-based metric is valid only when it names what score or rating is being observed, such as user satisfaction score with workout content. For satisfaction concepts, prefer user-reported satisfaction rate/count/score/rating instead of vague satisfaction wording. Avoid vague standalone metrics such as remains high, remains stable, maintain satisfaction, improved trust, improved user experience, better satisfaction, or increase engagement. Do not copy input success_metrics unless they satisfy this rule. Do not invent target numbers such as 10%, 20%, 30%, or 50% unless those numbers exist in the input. If no reliable metric is supported, return success_metrics: [] and add an open question for metric definition or target.",
        },
        ensure_ascii=False,
        indent=2,
    )
